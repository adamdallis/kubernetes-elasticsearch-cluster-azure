...
#Forked project
I have forked with project from https://github.com/pires/kubernetes-elasticsearch-cluster most credit should go to him. 

#Some changes from the original project
Removed the ingest Nodes, they have a very specific purpose so I removed them if you want to add them I have created a folder called unmainted and the deployments can be found there.
Used VolumeClaimTemplates to store data.
...
* The containers create a 2GB disk and then it is mounted within the ElasticSearch folder. The disk use the azure premium template, this may need to be change in the scenario that another vendor is used like AWS.
